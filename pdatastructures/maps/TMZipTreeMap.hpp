 ...
 * TODO: get rid of the lookup and make insert return a boolean that indicates if the key was present or not
 */
template<typename K, typename V, typename TM, template <typename> class TMTYPE>
class TMZipTreeMap : public TM::tmbase {
...
public:
    ...
    bool add(K key, V& value, const int tid=0) {
        return TM::template updateTx<bool>([=] () {
            if (recursiveFind(key, root.pload())) return false;
            Node* node = TM::template tmNew<Node>(key, value);
            return iterativeInsert(node);
        });
    }
    ...
    bool remove(K key, const int tid=0) {
        return TM::template updateTx<bool>([=] () {
            ...
            bool retval = iterativeDelete(key);
    ...
    bool contains(K key, const int tid=0) {
        return TM::template readTx<bool>([=] () {
            return (recursiveFind(key, root.pload()) != nullptr);
        });
    }

    V get(K key) {
        return TM::template readTx<V>([=] () {
            Node* node = recursiveFind(key, root.pload());
            if (node == nullptr) return (V)nullptr;
            return node->value.pload();
        });
    }
