...
row_t * txn_man::get_row(row_t * row, access_t type) {
...
#elif (CC_ALG == DL_DETECT || CC_ALG == NO_WAIT || CC_ALG == WAIT_DIE || CC_ALG == TWO_PL_SF)
		access->orig_data = (row_t *) _mm_malloc(sizeof(row_t), 64);
		access->orig_data->init(MAX_TUPLE_SIZE);
#endif
		num_accesses_alloc ++;
	...
	//printf("row=%p manager=%p\n", row, row->manager);
	rc = row->get_row(type, this, accesses[ row_cnt ]->data);

	// PEDRO: this is a test remove me
	//static uint64_t g_row_cnt = 0;
	//g_row_cnt++;
    //if (g_row_cnt%1000 == 0) rc = Abort;
...
#if ROLL_BACK && (CC_ALG == DL_DETECT || CC_ALG == NO_WAIT || CC_ALG == WAIT_DIE || CC_ALG == TWO_PL_SF)
	if (type == WR) {
...
#if (CC_ALG == NO_WAIT || CC_ALG == DL_DETECT) && ISOLATION_LEVEL == REPEATABLE_READ
	// This is needed only when in isolation level "repeatable-read"
	if (type == RD) {
...
// If the transaction is meant to abort, the 'rc' will be 'Abort' due to a failed lock (or failed read-set validation)
RC txn_man::finish(RC rc) {
	...
	if (rc == RCOK) {
	    ...
	    twoplsf::abortTxn(false); // Use 'false' to disable auto-rollbacks
	}
	cleanup(rc);
#else 
	cleanup(rc);
